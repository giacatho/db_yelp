...
//-----------------------------------------------------------------------------------------
// http://stackoverflow.com/a/16938481/1343667
//-----------------------------------------------------------------------------------------
function fGetBrowserInfo() {
    var ua, tem, M;
...
//-----------------------------------------------------------------------------------------
function fIsFullySupportedBrowser() {
    var vBrowser = fGetBrowserInfo();
    
    if (!vBrowser || !vBrowser.name || !vBrowser.version)
        return false;
    
    // NOte: The idea for Chrome and Firefox is to have the latest one, but because
    // we can not always know exactly what is the latest version. I select some version 
    // released half year ago, which can be changed.
    if (vBrowser.name == 'Chrome') 
    {
        if (parseInt(vBrowser.version) >= 40) 
            return true;
        return false;
    } 
    
    if (vBrowser.name == 'Firefox')
    {
        if (parseInt(vBrowser.version) >=30)
            return true;
        return false;
    }
    
    if (vBrowser.name == 'Safari')
    {
        if (fIsIOSDevice())
            return true;
        
        if (parseInt(vBrowser.version) >= 7)
            return true;
        
        return false;
    }
    
    if (vBrowser.name == 'IE' || vBrowser.name == 'MSIE') 
    {
        if (parseInt(vBrowser.version) > 9)
            return true;
